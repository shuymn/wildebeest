...
| 項目 | 状態 |
|------|------|
| ブランチ | `feat/phase1-2-migrations`（`origin/main` から diverge、**push済み**） |
...
| 実装 | Phase 1–2 migration SQL 生成済み、Lists API 基盤・`interaction_count` 管理コード着手済み（**PR作成済み**） |
| migration 後方互換方針 | **Stance A 確定**（2026-06-08）。正典 → `docs/data-model-migration.md` の "Backward-Compatibility Stance: A" |

### PR #724 の変更内容（`feat/phase1-2-migrations`）
...
migration SQL・schema 更新・Lists / interaction_count 実装をまとめてコミットし PR を作成する。
...
| 優先 | 対象 | 理由 |
|------|------|------|
| 🟡 | block / mute / bookmark / favourite / unreblog | 基本ソーシャル操作（Lists 基盤は完了） |
...
bookmark 操作での増減を追加する（follow / favourite / reblog は完了）。  
...
`docs/data-model-types.md` を `packages/backend/src/types/` に反映（`list.ts` は完了）。

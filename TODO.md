...
| 項目 | 状態 |
|------|------|
| ブランチ | `feat/phase1-2-migrations`（`origin/main` から diverge、**未 push**） |
...
| 実装 | Phase 1–2 migration SQL 生成済み、Lists API 基盤・`interaction_count` 管理コード着手済み（**未コミット**） |
| migration 後方互換方針 | **Stance A 確定**（2026-06-08）。正典 → `docs/data-model-migration.md` の "Backward-Compatibility Stance: A" |

### 未コミットの変更（`feat/phase1-2-migrations`）
